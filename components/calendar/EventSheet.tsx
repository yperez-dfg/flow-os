...
export default function EventSheet({ open, onClose }: EventSheetProps) {
  ...
  // Sync to selected date and reset form each time the sheet opens
  useEffect(() => {
    if (open) {
    ...
    >
      <div className="space-y-3">
        ...
        <button
          ...
          className="w-full flex items-center justify-between py-4 px-4 bg-[#F5F5F7] rounded-2xl active:scale-[0.98] transition-transform"
        >
          <span className="text-sm text-[#1D1D1F]">Remind me (15 min before)</span>
          <div className={`w-12 h-7 rounded-full transition-colors relative flex-shrink-0 ${notify ? 'bg-[#1560FF]' : 'bg-[#C7C7CC]'}`}>
            <div className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow transition-transform ${notify ? 'translate-x-6' : 'translate-x-1'}`} />
          </div>
        </button>
      </div>
    </BottomSheet>
